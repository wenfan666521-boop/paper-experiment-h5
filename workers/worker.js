...
async function handleChat(request, env) {
  ...
  try {
    ...
    if (!r.ok) {
      return new Response(JSON.stringify({ error: d.error?.message || 'Bailian API error', detail: d }), {
        status: 502, headers: { ...CORS, 'Content-Type': 'application/json' }
      });
    }

    // 应用级API返回：d.output?.text 或 d.output?.content
    const reply = d.output?.text || d.output?.content || d.output?.response || '抱歉，暂时无法回复。';
